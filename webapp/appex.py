...
# Compatibility shim: some Python environments may not expose pkgutil.find_loader
# Dash (dev tools) uses pkgutil.find_loader; if it's missing, provide a simple
# fallback that uses importlib.util.find_spec to locate a module and return
# the loader (or None). This avoids the AttributeError seen in some setups.
...
# Predicts class probabilities for the test dataset X_test.
# pipeline.predict_proba(X_test) returns a 2D array where each row corresponds to a sample,
# and the columns represent the probabilities for each class.
test['Probability'] = pipeline.predict_proba(X_test)[:,1] # Selects the second column, probabilities for the positive class, Target=1.
test['Target'] = test[TARGET] # TARGET='Survived'
